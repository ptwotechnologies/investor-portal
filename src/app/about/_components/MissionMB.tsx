      ...
      content: (
        <>
          ...
          <p className="text-base font-normal">
            ...
            lack of access. That&apos;s why rather than creating yet another
            portal, we focused on curating the essential parts of building a
      ...
      {sectionsData.map((section) => (
        <div key={section.id} className={`${section.bgColor} w-full`}>
          <div className="mx-auto py-12">
            ...
            <div className="mb-4 flex flex-col items-start px-6 md:mb-6">
              <div
                className={`${section.stripeColor} mr-4 h-12 w-12 md:h-16 md:w-16`}
              ></div>
              <h3
                className={`text-2xl leading-[50px] font-semibold ${section.titleColor} md:text-3xl`}
              >
                {section.title}
              </h3>
            </div>
            ...
            <div className={`${section.stripeColor} px-6 py-6`}>
              <h2
                className={`text-[32px] leading-[35px] font-semibold ${section.headingColor}`}
              >
                {section.heading}
              </h2>
            </div>
            ...
            {/* Content paragraphs */}
            <div
              className={`leading-0[35px] space-y-4 px-6 pt-4 text-lg font-medium ${section.contentColor}`}
            >
              {section.content}
            </div>
          </div>
